						...
						<AnimatePresence initial={false} custom={direction}>
							...
							>
								<Image
									...
									width={navigation ? 1280 : 1922}
									height={navigation ? 853 : 1282}
									priority
					...
					{loaded && (
						<div className="relative aspect-[3/2] max-h-full w-full">
							...
							<div className="absolute right-0 top-0 flex items-center gap-2 p-3 text-white">
								{navigation ? (
									<a
										href={`https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/v${currentImage.version}/${currentImage.public_id}.${currentImage.format}`}
									...
									<a
										href={`https://twitter.com/intent/tweet?text=Check%20out%20this%20pic!%0A%0A${encodeURIComponent(typeof window !== 'undefined' ? window.location.origin : '')}/p/${index}`}
										className="rounded-full bg-black/50 p-2 text-white/75 backdrop-blur-lg transition hover:bg-black/75 hover:text-white"
										target="_blank"
										title="Open fullsize version"
										rel="noreferrer"
								...
								<button
									onClick={() =>
										downloadPhoto(
											`https://res.cloudinary.com/${process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME}/image/upload/v${currentImage.version}/${currentImage.public_id}.${currentImage.format}`,
											`${index}.jpg`,
										)
									}
									className="rounded-full bg-black/50 p-2 text-white/75 backdrop-blur-lg transition hover:bg-black/75 hover:text-white"
